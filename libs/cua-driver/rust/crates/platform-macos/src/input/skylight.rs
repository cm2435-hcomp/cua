...
/// `true` when the two symbols needed by the v2 target-only SLPS make-key route
/// resolved. Unlike `is_focus_without_raise_available`, this deliberately
/// does not require or call the front-process SPI.
pub fn is_target_slps_make_key_available() -> bool {
    get_process_for_pid_fn().is_some() && post_event_record_to_fn().is_some()
}

pub const EVENT_RECORD_LEN: usize = 0xF8;
pub const SLPS_MAKE_KEY_MARKER: u8 = 0x01;
pub const SLPS_REMOVE_KEY_MARKER: u8 = 0x02;

/// Build the recovered SLPS make-key/remove-key record without selecting a destination.
/// The caller must post it only to the intended target process PSN.
pub fn build_slps_make_key_record(window_id: u32, marker: u8) -> [u8; EVENT_RECORD_LEN] {
    let mut record = [0u8; EVENT_RECORD_LEN];
    record[0x04] = EVENT_RECORD_LEN as u8;
    record[0x08] = 0x0D;
    record[0x3C..=0x3F].copy_from_slice(&window_id.to_le_bytes());
    record[0x8A] = marker;
    record
}

/// Post one recovered make-key/remove-key record to the target process only.
/// This deliberately does not defocus the real foreground process and never
/// calls a global event-posting or front-process primitive.
pub fn post_slps_make_key_record_to_pid(pid: pid_t, window_id: u32, marker: u8) -> bool {
    let Some(post) = post_event_record_to_fn() else {
        return false;
    };
    let Some(get_process) = get_process_for_pid_fn() else {
        return false;
    };
    let mut target_psn = [0u8; 8];
    if unsafe { get_process(pid, target_psn.as_mut_ptr().cast()) } != 0 {
        return false;
    }
    let record = build_slps_make_key_record(window_id, marker);
    unsafe { post(target_psn.as_ptr().cast(), record.as_ptr()) == 0 }
}
