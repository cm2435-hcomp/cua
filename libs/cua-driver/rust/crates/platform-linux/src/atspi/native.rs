...
fn runtime() -> &'static tokio::runtime::Runtime {
    ...
    RT.get_or_init(|| {
        tokio::runtime::Builder::new_multi_thread()
            // A stopped provider can occupy both its request task and zbus's
            // connection executor. Keep a third worker available so the 50 ms
            // observation deadline remains schedulable.
            .worker_threads(3)
            .enable_all()
            .build()
...
async fn app_for_pid(
    ...
    for child in apps {
        ...
        let app = match {
            let child = child.clone();
            topology_call(async move { accessible_for(conn, &child).await }).await
        } {
            Some(Ok(app)) => app,
            Some(Err(error)) => {
...
async fn resolve_window_frame(
    ...
    for (ordinal, oref) in seeds.iter().enumerate() {
        let Some(Ok(acc)) = ({
            let oref = oref.clone();
            topology_call(async move { accessible_for(conn, &oref).await }).await
        }) else {
            continue;
        };
